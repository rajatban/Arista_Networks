...
%         rxDataBits = wlanVHTDataRecover((rxWaveform_sum + noise),chanEstsum,noiseVarEst_sum,cfgVHT1,1);
        ...
        %% Calculating BER
        BER1(ii) = BER1(ii) + sum(xor(txDataBits,rxDataBits1))/length(txDataBits);
        BER2(ii) = BER2(ii) + sum(xor(txDataBits,rxDataBits2))/length(txDataBits);
    end
    disp(jj);
end
...
%% Save variables
filename = ['SimData-BER1_P-',num2str(P),'_d-',num2str(d),'_v-0.mat'];
save(filename, 'P', 'd', "BER1");
filename = ['SimData-BER2_P-',num2str(P),'_d-',num2str(d),'_v-0.mat'];
